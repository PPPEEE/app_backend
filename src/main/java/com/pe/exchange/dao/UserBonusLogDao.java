...
public interface UserBonusLogDao extends JpaRepository<UserBonusLog,Integer> {
    ...
    @Query(value = "select * from user_bonus_log where status=1 and user_id=:userId  and bonus_coin_type=1 and bonus_type=5 order by add_time desc limit :offset,:pageSize ",nativeQuery = true)
    List<UserBonusLog> findDNReleaseList(@Param("userId") Integer userId,@Param("offset") int offset,@Param("pageSize")int pageSize);

    @Query(value = "select * from user_bonus_log where status=1 and user_id=:userId  and bonus_coin_type=0 and bonus_type=0 order by add_time desc limit :offset,:pageSize ",nativeQuery = true)
    List<UserBonusLog> findDKForzenList(@Param("userId") Integer userId,@Param("offset") int offset,@Param("pageSize")int pageSize);

    @Query(value = "select * from user_bonus_log where status=1 and user_id=:userId  and bonus_coin_type=1 and (bonus_type=3 or bonus_type=4) order by add_time desc limit :offset,:pageSize ",nativeQuery = true)
