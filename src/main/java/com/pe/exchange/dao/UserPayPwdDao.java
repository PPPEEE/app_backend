...
public interface UserPayPwdDao extends JpaRepository<UserPayPwdInfo, Integer>{
	...
	@Query(value = "select * from user_pay_pwd_info where userId = :userId",nativeQuery=true)
	UserPayInfo queryPayPwdExists(@Param("userId")Integer userId);
	
	@Query(value = "select * from user_pay_pwd_info where userId = :userId and pwd = :pwd",nativeQuery=true)
