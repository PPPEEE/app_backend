...
public interface UserBalanceDao extends JpaRepository<UserBalance,Integer> {
    ...
    @Modifying
    @Query(value = "update user_balance set balance=balance-:amount where user_id=:userId  and balance>=:amount  and coin_type=0",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "update user_balance set balance=balance-:amount where user_id=:userId and balance>=:amount and coin_type=1",nativeQuery = true)
