...
public interface DKDealDao extends JpaRepository<DKDealInfo, Integer>{
	...
	@Query(value="select count(1) from dkdeal_info where type = :type and user_id != :userId",nativeQuery = true)
	Integer findTypeDKList(@Param("type")int type,@Param("userId")int userId);
	
	@Query(value="select * from dkdeal_info where type = :type and user_id != :userId  limit :page,:pageSize",nativeQuery = true)
	...
	@Query(value="select count(1) from dkdeal_info where user_id = :userId and type = :type",nativeQuery = true)
	Integer findUserDKList(@Param("userId")Integer userId,@Param("type") String type);
	
	
	@Query(value="select * from dkdeal_info where user_id = :userId and type = :type limit :page,:pageSize",nativeQuery = true)
